...
namespace
{
	...
	public:
		...
		Game()
		{
			...
			//swapChainDesc.SwapEffect = DXGI_SWAP_EFFECT_FLIP_SEQUENTIAL; //new but will require tweaks
			...
			viewport.TopLeftX = 0;
			viewport.TopLeftY = 0;
			//viewport.Width = static_cast<float>(gWidth); //using width/height of window creation seems to be larger; it isn't  the draw size but the drawsize+taskbar size
		...
		void gameLoop()
		{
			...
			//TODO bind states for display purposes
			//pDeviceContext->VSSetShader(pVertShader, nullptr, 0);
			//pDeviceContext->PSSetShader(pPixelShader, nullptr, 0);
			//pDeviceContext->IASetInputLayout(pVertLayout);
			//pDeviceContext->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
	...
	void gameLoop()
	{
		//create a game the first time this method is called (local static)
		static Game myGame;
